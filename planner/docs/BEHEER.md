...
landingspagina. Je hebt geen developer nodig voor scholen, vakken, periodes,
begeleiders of accounts.

## Tabbladen

### Inschrijvingen
Alle binnengekomen inschrijvingen, nieuwste bovenaan. Per rij zie je school,
jaarlaag, vakken, de periodevoorkeur en het contactadres. De statuskolom
(`nieuw` / `ingepland` / `afgerond` / `geannuleerd`) pas je hier direct aan.
Een inschrijving met status `geannuleerd` telt niet meer mee in de groepeer-hint.
...
- **Groepeer-hint.** Inschrijvingen gebundeld op school + jaarlaag + vak + periode
  + dagdeel. "Maak sessie(s)" maakt per groep een sessie aan en splitst
  automatisch zodra de groep groter is dan het ingestelde maximum.
- **Sessies.** Per sessie vul je datum, dagdeel, locatie en begeleider in en vink
  je de leerlingen aan. De begeleider-keuzelijst laat zien wie gekwalificeerd is
  voor dat vak en die jaarlaag. Wijzigingen sla je per sessie op.

### Begeleiders
Docenten, trainers en ondersteuners. Per begeleider: naam, e-mail, welke vakken
en jaarlagen die kan bedienen, de beschikbaarheid (datum + dagdeel) en het
maximum aantal sessies per weekend. Begeleiders kunnen hun beschikbaarheid ook
zelf bijwerken via hun eigen inlog (rol Begeleider).

### Scholen en vakken
De beheerbare lijsten: scholen, vakken, jaarlagen, periodes (met begin- en
einddatum) en de minimale/maximale groepsgrootte. Alles wat je hier toevoegt
verschijnt meteen in het inschrijfformulier.
